...
- **De a dos 👥 (dos teléfonos, sin servidores)**: el botón **De a dos** abre una partida para dos jugadores en la granja — uno maneja la **vaquita 🐄** y el otro el **perrito 🐶**, tirándose la pelota por turnos. La conexión es **directa entre los dos navegadores** (WebRTC), sin backend ni cuentas: el que crea la partida obtiene un **código QR** para que el otro escanee si están uno al lado del otro, o un **link para mandar por WhatsApp** si están lejos; el segundo devuelve su respuesta por la misma vía (QR o link) y arranca el juego. Como la física es determinista, por la red viajan **solo los tres números del tiro** y los dos ven exactamente la misma trayectoria; el anfitrión reparte además la cancha de cada turno para que el viento y las posiciones coincidan. El chip del HUD muestra el marcador y de quién es el turno. *Si están en redes móviles distintas puede no conectar (NAT): con los dos en el mismo WiFi anda siempre — y ahí ni siquiera hace falta internet.*
- **Living 🛋️**: recrea **el living de casa** en 3D, modelado a partir de fotos reales: el estar con los sillones crema y el sofá marrón, la mesa ratona, el **puf pelota Tango**, la tele, el rincón de juegos con su alfombra de calles, el comedor con la mesa oscura y las vitrinas blancas, los cuadros (¡incluida la noche estrellada!), y el hall con la escalera, el perchero y la puerta blanca. La estructura es procedural y **todos los muebles son glTF descargados** del [Furniture Kit de Kenney](https://kenney.nl/assets/furniture-kit) (CC0), teñidos con los colores de las fotos. Se orbita y zoomea como siempre, y se puede **recorrer por dentro con 🚶 Navegar** — ahí el paseo tiene **colisiones por raycast**: ni los muebles ni las paredes se atraviesan (dos rayos, a la rodilla y al pecho, con deslizamiento al chocar).
...
vendor/               three.js r147 + OrbitControls + GLTFLoader + QR
                      (qrcode-generator y jsQR), todos vendoreados
assets/models/        modelos glTF descargados (animales del modo Renders y
                      muebles Kenney CC0 del modo Living en living/)
